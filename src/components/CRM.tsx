...
export default function CRMComponent() {
  ...
  const handleExportPDF = () => {
    ...
    if (dateFilter.startDate || dateFilter.endDate) {
      ...
      filterText += `Date Range: ${dateFilter.startDate || 'Any'} to ${dateFilter.endDate || 'Any'}`;
    }
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
            <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto items-start sm:items-center">
              <span className="text-xs font-semibold text-gray-600 hidden sm:block">Date Range:</span>
              <div className="grid grid-cols-2 gap-2 w-full sm:w-auto">
                <input
                  type="date"
                  value={dateFilter.startDate}
                  onChange={(e) => setDateFilter({...dateFilter, startDate: e.target.value})}
                  className="w-full rounded-lg border-2 border-gray-300 p-2 text-xs focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none bg-white"
                />
                <input
                  type="date"
