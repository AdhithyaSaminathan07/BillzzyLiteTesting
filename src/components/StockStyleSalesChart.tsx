...
export default function StockStyleSalesChart() {
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 font-sanst">
      ...
      {/* FOOTER TABS */}
      <div className="flex justify-center gap-2 mt-6 border-t border-gray-100 pt-4">
        {TABS.map((tab) => (
          <button key={tab} onClick={() => setActiveTab(tab)} className={`text-xs font-black py-2 px-4 rounded-lg transition-all ${activeTab === tab ? "bg-gray-900 text-white shadow-lg" : "text-gray-400 hover:bg-gray-100"}`}>{tab}</button>
        ))}
      </div>
    </div>
  );
