...
// Custom interfaces to fix Recharts type issues
    ...
    return (
      <div className="bg-slate-900 text-white p-3 rounded-lg shadow-xl border border-slate-700 text-xs z-50">
        ...
        <p className="font-bold text-lg flex items-center gap-2">
          Bill: <span className="text-emerald-400">₹{amount.toLocaleString()}</span>
        </p>
      </div>
    );
  }
...
export default function StockStyleSalesChart() {
  ...
  if (loading && data.length === 0) {
    return (
      <div className="h-96 w-full bg-white rounded-xl flex items-center justify-center border border-gray-100">
        <Loader2 className="w-8 h-8 animate-spin text-gray-300" />
      </div>
    );
  }
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 font-sans">
      ...
      {/* GRAPH AREA */}
      <div className="h-60 w-full mt-4 -ml-4">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} 
            margin={{ top: 30, right: 35, left: 10, bottom: 0 }} 
            onMouseMove={(e) => { 
                const chartEvent = e as ChartMouseEvent;
                if (chartEvent && chartEvent.activePayload && chartEvent.activePayload.length > 0) {
                    setDisplayValue(chartEvent.activePayload[0].value); 
                }
            }} 
            onMouseLeave={() => setDisplayValue(null)}>
            <defs>
